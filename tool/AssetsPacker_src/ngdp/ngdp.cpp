...
const uint32 INVALID = 0xFFFFFFFFUL;
...
std::map<std::string, CdnData> GetCdns(std::string const &app) {
  // http://us.patch.battle.net:1119/w3/cdns

  // http.response.body:
  // Name!STRING:0|Path!STRING:0|Hosts!STRING:0|Servers!STRING:0|ConfigPath!STRING:0
  // ## seqn = 1476802
  // eu|tpr/war3|level3.blizzard.com
  // eu.cdn.blizzard.com|http://eu.cdn.blizzard.com/?maxhosts=4
  ...
  // https://level3.ssl.blizzard.com/?fallback=1&maxhosts=4|tpr/configs/data
  // kr|tpr/war3|level3.blizzard.com kr.cdn.blizzard.com
  // blizzard.gcdn.cloudn.co.kr|http://blizzard.gcdn.cloudn.co.kr/?maxhosts=4
  ...
  // https://level3.ssl.blizzard.com/?fallback=1&maxhosts=4|tpr/configs/data
  // us|tpr/war3|level3.blizzard.com
  // us.cdn.blizzard.com|http://level3.blizzard.com/?maxhosts=4
  // http://us.cdn.blizzard.com/?maxhosts=4
  // https://blzddist1-a.akamaihd.net/?fallback=1&maxhosts=4
  // https://level3.ssl.blizzard.com/?fallback=1&maxhosts=4
  // https://us.cdn.blizzard.com/?fallback=1&maxhosts=4|tpr/configs/data
  // tw|tpr/war3|level3.blizzard.com
  // us.cdn.blizzard.com|http://level3.blizzard.com/?maxhosts=4
  // http://us.cdn.blizzard.com/?maxhosts=4
  // https://blzddist1-a.akamaihd.net/?fallback=1&maxhosts=4
  // https://level3.ssl.blizzard.com/?fallback=1&maxhosts=4
...
std::map<std::string, VersionData> GetVersions(std::string const &app) {
  ...
  // ## seqn = 2270852
  // us|bbdd30f5d2a57df6ae6db87719c4248a|88da86fba1a62f60aabd5f0db4953ee0||21230|1.36.2.21230|f220f1a2d27b38e9469987ff66e61f04
  // eu|bbdd30f5d2a57df6ae6db87719c4248a|88da86fba1a62f60aabd5f0db4953ee0||21230|1.36.2.21230|f220f1a2d27b38e9469987ff66e61f04
  // cn|bbdd30f5d2a57df6ae6db87719c4248a|88da86fba1a62f60aabd5f0db4953ee0||21230|1.36.2.21230|f220f1a2d27b38e9469987ff66e61f04
  // kr|bbdd30f5d2a57df6ae6db87719c4248a|88da86fba1a62f60aabd5f0db4953ee0||21230|1.36.2.21230|f220f1a2d27b38e9469987ff66e61f04
  // tw|bbdd30f5d2a57df6ae6db87719c4248a|88da86fba1a62f60aabd5f0db4953ee0||21230|1.36.2.21230|f220f1a2d27b38e9469987ff66e61f04
...
NGDP::NGDP(std::string const &app, std::string const &region) {
  ...
  if (!cdns.count(region) || !versions.count(region)) {
    std::string oss = "invalid region: " + region;
    throw Exception(oss.c_str());
  }
...
std::string NGDP::geturl(std::string const &hash, std::string const &type,
                         bool index) const {
  std::string url = base_ + type + "/" + hash.substr(0, 2) + "/" +
                    hash.substr(2, 2) + "/" + hash;
  if (index)
    url += ".index";
  return url;
}
#include "utils/logger.h"
File NGDP::load(std::string const &hash, std::string const &type, bool index,
                char const *preload) const {
...
File DecodeBLTE(File blte, uint32 eusize) {
  if (blte.read32(true) != 0x424c5445 /*BLTE*/)
    return File();
  ...
  if (headerSize) {
    ...
    if (type == 'N') {
      ...
      MemoryFile dst;
      if (gzinflate(&tmp[0], size, dst.alloc(eusize), &eusize))
        return File();
      dst.seek(0);
      return dst;
    } else {
...
std::string to_string(const Hash hash) { return MD5::format(hash); }
...
Encoding::Encoding(File file) {
  ...
  if (header.signature != 0x454e /*EN*/ || header.sizeA != 16 ||
      header.sizeB != 16) {
  ...
  data_.resize(header.stringSize + (header.entriesA + header.entriesB) * 4096 +
               (size - posLayout));
  ...
  for (uint32 i = 0; i < header.entriesA; ++i) {
    ...
    for (uint8 *ptr = entriesA;
         ptr + sizeof(EncodingEntry) <= entriesA + 4096;) {
...
Encoding::EncodingEntry const *Encoding::getEncoding(const Hash hash) const {
  ...
  if (sub == it->entries.end())
    return nullptr;
  if (memcmp((*sub)->hash, hash, sizeof(Hash)))
    return nullptr;
  return *sub;
}
Encoding::LayoutEntry const *Encoding::getLayout(const Hash key) const {
  ...
  if (sub == it->entries.end())
    return nullptr;
  if (memcmp((*sub)->key, key, sizeof(Hash)))
    return nullptr;
  return *sub;
}

ArchiveIndex::ArchiveIndex(NGDP const &ngdp, uint32 blockSize)
    : ngdp_(ngdp), blockSize_(blockSize) {
  Hash nilHash;
  memset(nilHash, 0, sizeof(Hash));
  ...
  for (size_t i = 0; i < archives.size(); ++i) {
    ...
    Logger::item(archives_[i].name.c_str());
...
//        File(path::work() / CACHE / "data2" / name,
//        "wb").copy(File(path::work() / CACHE / "data" / name, "rb"));
//      }
//    } else if (!File::exists(path::work() / CACHE / "data" / name + ".index"))
//    {
//      File(path::work() / CACHE / "data2" / name, "wb").copy(File(path::work()
//      / CACHE / "data" / name, "rb"));
//    }
//  }
//}
...
File ArchiveIndex::load(Hash const &hash) {
  ...
  if (it == index_.end())
    return ngdp_.load(hash, "data");
  std::string archivePath =
      path::root() / CACHE / "data" / archives_[it->second.index].name;
  ...
  if (getStart < getEnd) {
    ...
    request.addHeader(fmtstring("Range: bytes=%u-%u", getStart * blockSize_,
                                getEnd * blockSize_ - 1));
    request.send();
    ...
    if (!archive)
      archive = File(archivePath, "wb+");
    if (headers.count("Content-Range")) {
      uint32 start, end, total, bstart, bend;
      if (sscanf(headers["Content-Range"].c_str(), "bytes %u-%u/%u", &start,
                 &end, &total) != 3) {
        return File();
      }
      ...
      map.resize((total + blockSize_ - 1) / blockSize_, INVALID);
      ...
      File(path::root() / CACHE / "data" / archives_[it->second.index].name +
               ".map",
           "wb")
          .write(&map[0], map.size() * sizeof(uint32));
    } else {
      archive.copy(result);
  ...
  for (uint32 i = blockStart; i < blockEnd; ++i) {
    ...
    uint32 curend =
        std::min<uint32>(offset + size, i * blockSize_ + blockSize_);
...
CascStorage::CascStorage(std::string const &root) : root_(root) {
  createDirectory("config");
  createDirectory("data");
  createDirectory("indices");
  createDirectory("patch");

  std::vector<std::string> names;
  listFilesInDirectory("data", names);
...
File &CascStorage::addConfig(std::string const &hash, File &file) {
  file.seek(0);
  File(root_ / "config" / hash.substr(0, 2) / hash.substr(2, 2) / hash, "wb")
      .copy(file);
  file.seek(0);
  return file;
}
...
DataStorage::DataStorage(CascStorage &storage)
    : storage_(storage), indexCount_(0), dataCount_(0) {}
...
uint32_t byteswap_ulong(uint32_t value) {
  return ((value << 24) & 0xFF000000u) | ((value << 8) & 0x00FF0000u) |
         ((value >> 8) & 0x0000FF00u) | ((value >> 24) & 0x000000FFu);
}
void DataStorage::writeIndex() {
  ...
  header.maxOffset = byteswap_uint64(MaxDataSize);
  ...
  std::sort(index_.begin(), index_.end(),
            [](IndexEntry const &lhs, IndexEntry const &rhs) {
  ...
  for (IndexEntry const &entry : index_) {
    ...
    *(uint32 *)(write.pos + 1) = byteswap_ulong(entry.offset);
...
/*
void DownloadGame(NGDP const& ngdp, std::string const& build, std::string const&
path, std::vector<std::string> const& tags) { CascStorage storage(path /
"Data");

  printf("fetching configuration...\n");
  auto cdnConfig = ParseConfig(storage.addConfig(ngdp.version().cdn,
ngdp.load(ngdp.version().cdn))); auto buildConfig =
ParseConfig(storage.addConfig(build, ngdp.load(build)));
  ...
  if (!rawFile) rawFile = storage.addArchive(encodingCdnHash,
ngdp.load(encodingCdnHash, "data")); Encoding
encoding(DecodeBLTE(data.addFile(hash, rawFile))); rawFile.release();
  ...
  if (!rawFile) rawFile = storage.addArchive(to_string(enc->keys[0]),
ngdp.load(enc->keys[0], "data")); File download =
DecodeBLTE(data.addFile(enc->keys[0], rawFile), enc->usize); rawFile.release();
  ...
  for (uint32 i = 0; i < dlEntries; ++i) {
    ...
    if (dlErrors) {
      printf("\r%u/%u %I64u/%I64uM [%d failed]", dlGotEntries++,
dlMaskedEntries, dlGotSize >> 20, dlTotalSize >> 20, dlErrors); } else {
      printf("\r%u/%u %I64u/%I64uM", dlGotEntries++, dlMaskedEntries, dlGotSize
>> 20, dlTotalSize >> 20);
    }
  ...
  rawFile = storage.getArchive(to_string(enc->keys[0]));
  if (!rawFile) rawFile = storage.addArchive(to_string(enc->keys[0]),
ngdp.load(enc->keys[0], "data")); File install =
DecodeBLTE(data.addFile(enc->keys[0], rawFile), enc->usize); rawFile.release();
